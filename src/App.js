...
const ZeroKnowledgeAuth = () => {
  ...
  const Cancel = () => <h1>Payment Canceled. Please try again.</h1>;
  ...
  const generateKeys = async () => {
    ...
    const privateKeyBase64 = btoa(
      String.fromCharCode(...new Uint8Array(privateKeyPem))
    );
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!challengeResponse.ok) {
        throw new Error(
          `Failed to fetch challenge: ${challengeResponse.status}`
        );
      }
      ...
      if (!verificationResponse.ok) {
        throw new Error(
          `Failed to verify signature: ${verificationResponse.status}`
        );
      }
